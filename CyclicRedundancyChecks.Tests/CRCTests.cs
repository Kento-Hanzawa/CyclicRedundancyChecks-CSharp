...
namespace CyclicRedundancyChecks.Tests
{
    public class CRCTests
    {
        ...
        public void CRC32_RefIn(string source)
        {
            ...
            {
                var hash = crc.ComputeHash(data);
                Assert.Equal(0x47DEA658.ToString("X8"), BitConverter.ToUInt32(hash).ToString("X8"));
            }
        }
        ...
        public void CRC32_RefOut(string source)
        {
            ...
            {
                var hash = crc.ComputeHash(data);
                Assert.Equal(0xE7485EDD.ToString("X8"), BitConverter.ToUInt32(hash).ToString("X8"));
            }
        }
        ...
        public void CRC32(string source)
        {
            ...
            {
                var hash = crc.ComputeHash(data);
                Assert.Equal(0xBB7A12E7.ToString("X8"), BitConverter.ToUInt32(hash).ToString("X8"));
            }
        }
    }
}
