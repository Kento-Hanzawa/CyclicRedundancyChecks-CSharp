...
namespace CyclicRedundancyChecks
{
    ...
    public sealed class CRC8 : CRC
    {
        ...
        private const int MaxBitWidth = sizeof(byte) * 8;

        private byte _result;
        private byte[] _lookupTableBuffer;
        ...
        private byte BitMask => (BitWidth == MaxBitWidth) ? (byte)0xFF : (byte)((1UL << BitWidth) - 1);
        private int BitWidthInBytes => sizeof(byte);

        private ReadOnlySpan<byte> LookupTable
        {
            get
            {
                if (_lookupTableBuffer is null)
                {
                    _lookupTableBuffer = ArrayPool<byte>.Shared.Rent(LookupTableSize);
        ...
        /// <summary>
        /// 各パラメータを指定して、<see cref="CRC8"/> クラスの新しいインスタンスを作成します。
        /// </summary>
        ...
        /// </param>
        public CRC8(byte polynomial, byte initialValue, byte finalXorValue, bool reflectInput, bool reflectOutput, bool isBigEndian = false, bool useLookupTable = true)
            : this(polynomial, initialValue, finalXorValue, reflectInput, reflectOutput, 8, isBigEndian, useLookupTable) { }

        internal CRC8(byte polynomial, byte initialValue, byte finalXorValue, bool reflectInput, bool reflectOutput, int bitWidth, bool isBigEndian = false, bool useLookupTable = true)
        {
...
#pragma warning disable CS1591
        ...
        protected sealed override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                {
                    ArrayPool<byte>.Shared.Return(_lookupTableBuffer);
