...
namespace CyclicRedundancyChecks
{
    ...
    public sealed class CRC64 : CRC
    {
        ...
        private const int MaxBitWidth = sizeof(ulong) * 8;

        private ulong _result;
        private ulong[] _lookupTableBuffer;
        ...
        private ulong BitMask => (BitWidth == MaxBitWidth) ? 0xFFFF_FFFF_FFFF_FFFF : (1UL << BitWidth) - 1;
        private int BitWidthInBytes => sizeof(ulong);

        private ReadOnlySpan<ulong> LookupTable
        {
            get
            {
                if (_lookupTableBuffer is null)
                {
                    _lookupTableBuffer = ArrayPool<ulong>.Shared.Rent(LookupTableSize);
        ...
        /// <summary>
        /// 各パラメータを指定して、<see cref="CRC64"/> クラスの新しいインスタンスを作成します。
        /// </summary>
        ...
        /// </param>
        public CRC64(ulong polynomial, ulong initialValue, ulong finalXorValue, bool reflectInput, bool reflectOutput, bool isBigEndian = false, bool useLookupTable = true)
            : this(polynomial, initialValue, finalXorValue, reflectInput, reflectOutput, 64, isBigEndian, useLookupTable) { }

        internal CRC64(ulong polynomial, ulong initialValue, ulong finalXorValue, bool reflectInput, bool reflectOutput, int bitWidth, bool isBigEndian = false, bool useLookupTable = true)
        {
...
#pragma warning disable CS1591
        ...
        protected sealed override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                {
                    ArrayPool<ulong>.Shared.Return(_lookupTableBuffer);
