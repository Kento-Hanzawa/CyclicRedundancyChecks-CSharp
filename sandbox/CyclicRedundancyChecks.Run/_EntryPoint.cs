//
// Console Application Template-1.0.7
//
...
#pragma warning restore RS0030 // 禁止された API を使用しない
    ...
    await Host.CreateDefaultBuilder()
        .ConfigureServices(static (HostBuilderContext context, IServiceCollection service) =>
        {
            ...
            var fileInfo = new FileInfo(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, CyclicRedundancyChecks.Run.AppSettings.FileName));
            if (!fileInfo.Exists)
            {
                var settings = new Dictionary<string, object> { [context.HostingEnvironment.ApplicationName] = new CyclicRedundancyChecks.Run.AppSettings(), };
            ...
            service.Configure<CyclicRedundancyChecks.Run.AppSettings>(builder.Build().GetSection(context.HostingEnvironment.ApplicationName));
        })
...
#if DEBUG
        .RunConsoleAppFrameworkWebHostingAsync(CyclicRedundancyChecks.Run.AppMain.SwaggerUrl);
#else
        .RunConsoleAppFrameworkAsync<CyclicRedundancyChecks.Run.AppMain>(args);
#endif
}
finally
{
